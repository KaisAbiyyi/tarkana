...
async function processDir(dir) {
    ...
    for (const entry of entries) {
        if (entry.name === 'node_modules' || entry.name === '.svelte-kit' || entry.name.startsWith('dashboard-first-run')) continue;
        ...
        if (entry.isDirectory()) {
            ...
            if (content.includes('publicDiscriminator') || content.includes('public_discriminator')) {
                ...
                content = content.replace(/publicDiscriminator:[ \t]*varchar\('public_discriminator'.*$/gm, '');

	...
	switch (rule) {
		...
		case 'shape_order': {
			...
			return {
				visible: values.slice(0, 5),
				answer: values[5] as string,
				explanation: 'Shape order: the sequence follows the configured triangle direction order.'
			};
		}
		case 'growing_count': {
			const start = rng.intBetween(0, SHAPES.length - 1);
			...
			return {
				...
				answer: values[5] as string,
				explanation:
					'Growing count: each triangle direction appears one more time before the direction changes.'
			};
		}

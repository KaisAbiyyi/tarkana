...
async function fix() {
	let content1 = await fs.readFile(
		'src/lib/server/sessions/active-challenge-service.spec.ts',
		'utf8'
	);
	...
	let content2 = await fs.readFile(
		'src/lib/server/sessions/abandon-challenge-service.spec.ts',
		'utf8'
	);
	...
	content2 = content2.replace(
		/createProfileRepositoryFake\(\{ id: 'user-1' \}\)/g,
		"createProfileRepositoryFake(createProfile({ id: 'user-1' }))"
	);
	await fs.writeFile('src/lib/server/sessions/abandon-challenge-service.spec.ts', content2);

	let content3 = await fs.readFile(
		'src/routes/(app)/challenge/challenge-page.svelte.spec.ts',
		'utf8'
	);
